    ...
    >
      {allEmpty && (
        <header className="mb-10">
          <h1 className="text-4xl text-primary leading-tight mb-2" style={{ fontFamily: "'Instrument Serif', serif" }}>
      ...
      {template.slots.map((slot, idx) => {
        ...
        return (
          <section key={slot.id} className={idx === 0 ? "" : "mt-10"}>
            ...
            {isEmpty ? (
              <p className="text-[14px] text-secondary/60 italic leading-relaxed">{slot.prompt}</p>
            ) : (
...
function BlockRow({
  ...
  return (
    <div
      className={`group relative ${isPending ? "border-l-2 border-emerald-500/70 pl-3 py-0.5" : ""}`}
    ...
    >
      ...
      {isPending && (
        <div className="mt-1.5 flex items-center gap-3 text-[12px]">
          ...
          {block.rationale && <span className="text-emerald-700/70 italic">— {block.rationale}</span>}
        </div>
      )}
    </div>
  );
